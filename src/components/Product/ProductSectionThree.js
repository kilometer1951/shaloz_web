...
import ShopsInterestedIn from "./ShopsInterestedIn";
...
const ProductSectionTwo = (props) => {
  ...
  const renderItems =
    fetchedProductsCategory.otherProducts != undefined &&
    fetchedProductsCategory.otherProducts.map((result, index) => {
        ...
        <div
          ...
          <div>
            <p style={{ fontFamily: "poppins_light" }}>
              {truncateString(result.product_name, 35)}
            </p>
          </div>
          <div>
            <p
              ...
              {truncateString(result.seller[0].shop_name, 35)}
            </p>
          </div>
  ...
  return (
    <div>
      ...
      <div className="cat-see-more">
        <button onClick={() => (window.location.href = "/product/all")}>
          See more
          <i className="fal fa-arrow-right"></i>
        </button>
      </div>
      <ShopsInterestedIn shopsData={fetchedProductsCategory.shops} />
    </div>
  );
};
